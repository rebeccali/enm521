...
\begin{align*}
F(\lambda) = \frac{1}{\sqrt{2\pi}} \int_{-\infty}^{\infty} e^{i \lambda z} f(\tau) d\tau
\end{align*}
...
\begin{align}
F_n(\lambda) = \mathcal{F} \{f^{(n)}(t)\} = (-i\lambda)^n F(\lambda)
\end{align}
...
Thus, we define a Fourier transofrm of $\phi$ with respect ot $x$:
...
We let $g(x) = \phi(x,t)$. Then, let $g(x) = B e^{-\beta x^2}$. 
